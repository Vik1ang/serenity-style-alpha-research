...
这是一个面向多 agent 的 structural alpha research skill / reference 包，可供 Codex、Claude、Gemini 或其他 Markdown-aware agent 使用。它把供应链瓶颈研究与全球科技周期、客观证据、流动性、低 ROE/低基数弹性约束合并为一条研究链：
**周期/技术通胀 → 确定性需求 → 受限供给 → 比较优势 → 低关注/误分类 → 价值传导/低基数弹性 → 催化 → 客观反证**。
默认输出语言为中文。

本仓库不是可运行应用，没有构建、测试或服务进程。仓库里的主交付物是 canonical skill 本身：
`skills/structural-alpha-research/`。`.codex/skills/structural-alpha-research/`
...
.
├── skills/
│   └── structural-alpha-research/    # canonical，可读/可分发的 skill 包
...
├── .codex/
│   └── skills/
│       └── structural-alpha-research/ # Codex 本地安装镜像
├── docs/
...
```bash
# 项目级安装
cp -r skills/structural-alpha-research \
...
本仓库中，`.codex/skills/structural-alpha-research/` 保持为本地 Codex 加载镜像。
维护时需要让 `skills/structural-alpha-research/` 和 `.codex/skills/...` 同步。

### Claude / Gemini / 其他 agent

- Claude Code：读 `CLAUDE.md`，再按任务加载 `skills/structural-alpha-research/SKILL.md` 和相关 `references/`。
...
- **框架验证**：修改 skill 或质疑框架有效性时，用正例、拥挤边界、暴露不纯、红旗反例的混合篮子测试；验证记录放在 `docs/decisions/`，不进入 runtime references。

结论只使用文本状态和置信度，不使用 A/B/C/D 评级。交易推演只做情景分析，不输出直接买入/卖出/仓位/目标价/止损价建议。

## A 股研究栈

A 股方法论不是美股 structural alpha 框架的逐条翻译，而是一套独立栈。
...
9. `a-share-4-pillars-*.md` — R1 来源、review 和 peer discussion，只在方法论 review 时加载。
...
- `skills/structural-alpha-research/SKILL.md` 是 canonical skill 入口，frontmatter 不能删。
- `.codex/skills/structural-alpha-research/` 是本地 Codex 镜像；维护时必须与 `skills/` 同步。
...
- skill 自身大改后，用混合验证篮子检查是否会把“好公司/好赛道/高 backlog”误判成强 Alpha；具体样本只放 `docs/decisions/`。
- 外部 cycle-method 项目只作为方法 benchmark；本仓库不内置外部观点语料，不得把框架推演伪装成来源归因观点。
