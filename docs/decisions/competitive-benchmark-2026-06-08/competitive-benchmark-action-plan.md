...
2. **P1-8 不抄 spikeHongg** —— serenity 自己的 6 模式: 多 thesis / 并购载体 / Biotech BD / 大市值重估 / 题材接力 / 普通 chokepoint
...
| **改什么** | `skills/serenity-style-alpha-research/SKILL.md` §"Stock-Picking Workflow" + `skills/serenity-style-alpha-research/references/a-share-skill-spec-FINAL.md` §"入口" + 同步更新 `.codex/skills/...` 镜像 + README / CLAUDE / GEMINI |
...
| **改什么** | **新增** `skills/serenity-style-alpha-research/references/a-share-r11-candidate-registry-lifecycle.md` (论文级, 6 块: thesis_id 规则 / 状态机 / 证据截止日 / checkpoint / 复盘 / 跟 R4-R10 引用) |
...
| **改什么** | **新增** `skills/serenity-style-alpha-research/references/citation-standard.md` (收窄版, 不照搬 spikeHongg); `SKILL.md` "Data Discipline" 段加引用硬规则 |
...
| **改什么** | **新增** `skills/serenity-style-alpha-research/references/data-status-contract.md` + `trading-scenario.md` "Required Inputs" 段 (~11-23 行) 加状态字段; `SKILL.md` "Data Discipline" 段加契约 |
...
| **改什么** | **新增** `skills/serenity-style-alpha-research/references/discovery-monitoring-pipeline.md` + `.github/workflows/serenity-research-digest.yml`; `SKILL.md` "Response Depth" 加 "定期候选扫描/跟踪摘要" 模式; `output-template.md` 加 "状态变化摘要" |
...
#### P1-8 Serenity 自定义 routing 矩阵 (6 模式, 不抄 spikeHongg)

| 字段 | 内容 |
|---|---|
| **来源** | spikeHongg routing-matrix (7 pattern) 借鉴**结构**; serenity 自己的 5 因子 + 5 桶 |
| **改什么** | **新增** `skills/serenity-style-alpha-research/references/routing-matrix.md` (6 模式) + `SKILL.md` "Stock-Picking Workflow" 段引用 |
...
| **改什么** | `skills/serenity-style-alpha-research/references/scoring.md` 拆"发现分"和"研究置信度"两块; **新增** `skills/serenity-style-alpha-research/references/discovery-signals.md` |
...
| **改什么** | `skills/serenity-style-alpha-research/references/output-template.md` "Compact Formats" 后新增 "One-page Investment Research Record"; 复用 P0-2 Registry 字段 (不另建状态体系) |
...
| # | 风险 | 等级 | 落地方式 |
|---|---|---|---|
| R-12 | 不学 ZhuLinsen 买卖信号 | 硬约束 | serenity 输出层 hard rule: 永远只出 5 档非建议状态 |
